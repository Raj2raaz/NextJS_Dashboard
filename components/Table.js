...
//   // ✅ Add a New Row
//   const handleAddRow = () => {
//     setEditedData([...editedData, Array(editedData[0].length).fill("")]);
//   };
...
//       refreshData(); // ✅ Refresh Data from Google Sheets
//     } catch (error) {
...
export default function Table({ data, refreshData }) {
  ...
  useEffect(() => {
    setEditedData(data);
  }, [data]); // ✅ Update table if data changes (e.g., column deletion)
  ...
  const handleSave = async () => {
    try {
      await axios.post("/api/google/updateData", { data: editedData });
      alert("Data updated successfully!");
      if (refreshData) refreshData(); // ✅ Ensure refreshData is defined
    } catch (error) {
      console.error("Error updating data:", error);
      alert("Failed to update data.");
    }
